...
Vendored under `data/` in git (~135MB). Absent DB returns restore hint
(`data/README.md`, `scripts/ensure_data.ps1`). Maintainers refresh with
`scripts/vendor_from_donor.ps1`. Agents: surface the hint, do not fake entries.
The quiz and kanji tools work fully without the big DB; vocab and examples need it.
