...
export function ToolsHarnessExplainer({ variant = "full" }: { variant?: Variant }) {
	if (variant === "compact") {
		return (
			<p className="text-xs text-zinc-500 border border-zinc-700 rounded-md bg-zinc-900/60 px-3 py-2">
				This is the <strong className="text-zinc-300">MCP tool harness</strong> — same
				tools and parameters Cursor or Claude use. <strong className="text-zinc-300">Run tool</strong>{" "}
				calls <code className="text-blue-400">POST /api/tools/…</code> on this server.
	...
	if (variant === "dashboard") {
		return (
			<div className="rounded-lg border border-zinc-700 bg-zinc-900/50 p-4 text-xs text-zinc-400 space-y-2">
				<p className="font-medium text-zinc-200">Why MCP Tools instead of more pages?</p>
				<p>
					japanophile-mcp exposes five portmanteau tools on one server. The harness
					introspects them so the webapp never drifts from MCP.
				</p>
	...
	return (
		...
		>
			...
			<p>
				Each card is one registered MCP tool. <strong className="text-white">Run tool</strong>{" "}
				builds a form from JSON Schema, then executes via{" "}
				<code className="text-blue-400 text-xs">POST /api/tools/{`{name}`}</code> — the same
				surface agents use.
			</p>
			<p className="text-xs text-zinc-500">
				...
				, and Games are tailored browse UIs. Kanji/JLPT/vocab/knowledge portmanteaus also run here
