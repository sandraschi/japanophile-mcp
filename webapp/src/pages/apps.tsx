...
export function AppsPage() {
	...
	useEffect(() => {
		...
		let cancelled = false;
		const ports = filtered.map((a) => a.port).filter((p) => p > 0);
		void mapPool(ports, 8, async (port) => {
			if (cancelled) return;
			try {
				const r = await fetch(`${API_BASE}/api/apps/health?port=${port}`);
				const h = (await r.json()) as Health;
				if (!cancelled) {
					setHealth((prev) => ({ ...prev, [port]: h }));
				}
			} catch {
				if (!cancelled) {
					setHealth((prev) => ({
	...
	return (
		<div className="space-y-6" data-testid="apps-page">
			...
			{loading && (
				...
				>
					<Loader2 className="w-5 h-5 animate-spin text-gh-green" />
					<span className="text-sm">Loading fleet registry…</span>
				</div>
			)}
			...
			{!loading && filtered.length === 0 && !error && (
				<p className="text-sm text-muted-foreground text-center py-8">
					{apps.length === 0
						? "No fleet apps with webapp ports in registry."
