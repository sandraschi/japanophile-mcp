...
/** Settings provider Test: Ollama includes llama-server inference smoke test. */
export function fetchLlmTest(
	provider: string,
	opts?: { model?: string; endpoint?: string },
): Promise<LlmTestResult> {
	const qs = new URLSearchParams({ provider });
	if (opts?.model?.trim()) qs.set("model", opts.model.trim());
	if (opts?.endpoint?.trim()) qs.set("endpoint", opts.endpoint.trim());
	return apiGet<LlmTestResult>(`/api/llm/test?${qs.toString()}`);
}

/** First usable model for chat when Settings saved provider only (no auto-pick in onboarding). */
export async function resolveDefaultModel(provider: string): Promise<string> {
	const sel = loadSelection();
	if (sel.provider === provider && sel.model.trim()) return sel.model.trim();
	try {
		const s = await fetchLlmSettings();
		if (s.provider === provider && s.model?.trim()) return s.model.trim();
	} catch {
		/* local mirror only */
	}
	const m = await fetchModels(provider);
	return m.models[0]?.trim() ?? "";
}
...
export function fetchLoaded(
	...
	const qs = `?provider=${encodeURIComponent(provider)}${endpoint ? `&endpoint=${encodeURIComponent(endpoint)}` : ""}`;
